...
class Settings:
    ...
    prompt_llm: LLMConfig = field(default_factory=lambda: LLMConfig(
    ...
    image_style_prompt: str = os.getenv("IMAGE_STYLE_PROMPT", "cinematic, dramatic lighting, detailed, illustration, anime style, 8k")

    # 音频生成配置
    edge_tts_voice: str = os.getenv("EDGE_TTS_VOICE", "zh-CN-YunxiNeural")
    audio_threads: int = int(os.getenv("AUDIO_THREADS", 5))

    # 视频生成配置
    video_threads: int = int(os.getenv("VIDEO_THREADS", 4))
    video_width: int = int(os.getenv("VIDEO_WIDTH", 750))
    video_height: int = int(os.getenv("VIDEO_HEIGHT", 1280))
    
    # 性能与流程控制
    max_llm_threads: int = int(os.getenv("LLM_THREADS", 3))
