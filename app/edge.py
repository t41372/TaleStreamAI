...
class EdgeTTSGenerator:
    ...
    def __init__(self, voice: str = DEFAULT_VOICE, rate: str = "+0%", volume: str = "+0%", pitch: str = "+0Hz"):
        self.voice = voice
        self.rate = rate
        self.volume = volume
        self.pitch = pitch
    ...
    async def generate_audio_with_subtitles(
        ...
        async with self.semaphore:
            try:
                ...
                await communicate.save(audio_path, None)
                
                # Generate subtitles from metadata
                subtitle_content = await self._extract_subtitles_from_stream(communicate, original_text)
    ...
    async def _extract_subtitles_from_stream(
        self, 
        communicate: edge_tts.Communicate, 
        original_text: Optional[str] = None
    ) -> str:
        """Extract subtitles from Edge TTS stream and restore punctuation if possible."""
        subtitles = []
        word_timings = []
        ...
        if original_text:
            subtitles = self._restore_punctuation_to_subtitles(word_timings, original_text)
        else:
    ...
    def _restore_punctuation_to_subtitles(
        ...
        for sentence in original_sentences:
            ...
            while word_idx < len(word_timings) and words_matched < len(sentence_words):
                ...
                sentence_end = word_timings[word_idx]["start"] + word_timings[word_idx]["duration"]
                word_idx += 1
                words_matched += 1
    ...
    def _group_words_into_subtitles(self, word_timings: List[Dict]) -> List[Dict]:
        ...
        for i, timing in enumerate(word_timings):
            ...
            if len(current_subtitle["words"]) >= words_per_subtitle or i == len(word_timings) - 1:
...
def sync_generate_audio_with_edge_tts(
    ...
    """
    return asyncio.run(generate_audio_with_edge_tts(
