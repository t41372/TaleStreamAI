...
async def _fetch_image(prompt: str) -> bytes:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            log_error(f"Image generation API request failed for prompt '{prompt[:30]}...': {e}")
            raise
...
async def _generate_image_asset(shot: Shot, book_path: Path) -> Shot:
    ...
    if shot.image_path.exists():
         log_debug(f"Image already exists for shot {shot.get_full_id()}, skipping generation.")
         return shot
...
async def _generate_audio_asset(shot: Shot, book_path: Path) -> Shot:
    ...
    if shot.audio_path.exists():
        log_debug(f"Audio already exists for shot {shot.get_full_id()}, skipping generation.")
        return shot
...
def _generate_video_clip_asset_sync(shot: Shot, book_path: Path) -> Shot:
    ...
    if shot.error or not shot.image_path or not shot.audio_path:
        log_warning(f"Skipping video clip for shot {shot.get_full_id()} due to previous errors or missing assets.")
        return shot
    ...
    try:
        ...
        with AudioFileClip(str(shot.audio_path)) as audio_clip, \
             ImageClip(str(shot.image_path)).set_duration(audio_clip.duration) as image_clip:
            ...
            zoomed_image = image_clip.resize(height=int(final_size[1] * 1.15)).set_position(("center", "center"))
            
            # Animate the zoom over the duration of the clip
            final_video = zoomed_image.resize(lambda t: 1 + 0.15 * (1 - t / audio_clip.duration))
        ...
        log_error(f"Shot {shot.get_full_id()} failed during video synthesis: {e}", exc_info=True)
...
async def generate_all_assets(
    ...
    log_info(f"Starting parallel generation of I/O-bound assets (image, audio) for {len(shots)} shots...")
