...
async def _merge_srt_files(sorted_shots: list[Shot], output_srt_path: Path):
    ...
    for shot in sorted_shots:
        ...
        try:
            ...
            log_warning(f"无法获取视频时长 {shot.video_clip_path.name}: {e}, 跳过此片段的字幕处理。")
            continue

        # 处理SRT文件
        if shot.srt_path and shot.srt_path.exists() and shot.srt_path.stat().st_size > 0:
            try:
                ...
                for sub in subs:
                    ...
                    subtitle_index += 1
                log_debug(f"已处理字幕 {shot.srt_path.name}, 添加了 {len(subs)} 条字幕。")
            except Exception as e:
