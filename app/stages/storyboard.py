...
def _split_content_by_tokens(content: str, model_name: str, max_tokens: int) -> list[str]:
    """按token数量将文本分割成块。"""
    log_debug(f"Splitting content by tokens, max_tokens={max_tokens}")
    try:
        encoding = tiktoken.encoding_for_model(model_name)
    except KeyError:
        log_warning(f"Model '{model_name}' not found for tiktoken, using 'cl100k_base'.")
    ...
    for p in paragraphs:
        ...
        if p_tokens > max_tokens:
            log_warning(f"Paragraph with {p_tokens} tokens exceeds max_tokens {max_tokens}, force splitting.")
...
async def _refine_shot_prompt(shot: Shot) -> Shot:
    ...
    if not shot.storyboard_prompt_en:
        ...
        log_warning(f"Skipping prompt refinement for shot {shot.get_full_id()}: No English storyboard prompt.")
        return shot

    messages = [{"role": "user", "content": shot.storyboard_prompt_en}]
    refined_prompt = await prompt_client.chat_completion(messages, system=PROMPT_REFINE_SYSTEM_PROMPT)

    if "ERROR" in refined_prompt:
        shot.error = f"Prompt refinement failed: {refined_prompt}"
        log_error(f"Failed to refine prompt for shot {shot.get_full_id()}: {refined_prompt}")
...
async def _process_chapter_content(chapter: Chapter) -> list[Shot]:
    ...
    for i, chunk in enumerate(content_chunks):
        log_info(f"Processing chunk {i+1}/{len(content_chunks)} for chapter {chapter.index}...")
        messages = [{"role": "user", "content": chunk}]
        response_json_str = await storyboard_client.chat_completion(messages, system=STORYBOARD_SYSTEM_PROMPT)

        if "ERROR" in response_json_str:
            log_error(f"Storyboard generation failed for chunk {i+1} of chapter {chapter.index}: {response_json_str}")
            continue
        
        try:
            # Clean up the JSON string before parsing
            response_json_str = re.sub(r'```json\s*|\s*```', '', response_json_str).strip()
                ...
                if isinstance(storyboard_data, dict):
                    ...
                    for key, value in storyboard_data.items():
                        if isinstance(value, list):
                            ...
                            found_list = True
                            log_warning(f"LLM returned a dictionary, but a list was found and extracted from key '{key}'.")
                            break
                    if not found_list:
                        raise json.JSONDecodeError("Response is not a list and no list found in dictionary values", response_json_str, 0)
                else:
                    raise json.JSONDecodeError("Response is not a list", response_json_str, 0)
            ...
            log_error(f"JSON parsing failed for chunk {i+1} of chapter {chapter.index}: {e}")
...
async def create_storyboard_for_chapters(chapters: list[Chapter]) -> list[Shot]:
    ...
    all_shots: list[Shot] = [shot for chapter_shots in results_per_chapter for shot in chapter_shots]
    
    log_info(f"Storyboard and prompting stage complete. Total shots created: {len(all_shots)}")
    return all_shots 