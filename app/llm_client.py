...
class LLMClient:
    ...
    def __init__(self, 
        ...
        log_debug(f"初始化 {client_type} 客戶端", 
    ...
    def chat_completion(self, 
        ...
        log_api_start(f"{self.client_type.upper()}_LLM", 
                     endpoint=self.base_url, model=model)
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                log_api_success(f"{self.client_type.upper()}_LLM", 
                               duration=duration,
                ...
                log_api_error(f"{self.client_type.upper()}_LLM", 
                             str(e), retry_count=attempt + 1)
    ...
    async def async_chat_completion(self, 
        ...
        log_api_start(f"ASYNC_{self.client_type.upper()}_LLM", 
                     endpoint=self.base_url, model=model)
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                duration = time.time() - start_time
                log_api_success(f"ASYNC_{self.client_type.upper()}_LLM", 
                               duration=duration,
                               details=f"嘗試次數: {attempt + 1}")
                ...
                log_api_error(f"ASYNC_{self.client_type.upper()}_LLM", 
                             str(e), retry_count=attempt + 1)
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                log_api_error(f"{self.client_type.upper()}_CONNECTION_TEST", 
                             f"響應內容不符合預期: {content}")
    ...
    async def async_test_connection(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                log_api_error(f"ASYNC_{self.client_type.upper()}_CONNECTION_TEST", 
                             f"響應內容不符合預期: {content}")
...
async def async_chat_with_semaphore(client: LLMClient, 
                                   messages: list,
...
async def batch_async_chat_completion(client: LLMClient,
    ...
    log_info(f"批量LLM調用完成 | 成功: {success_count} | 失敗: {failed_count} | 總耗時: {duration:.2f}s")
