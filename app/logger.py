...
class TaleStreamLogger:
    ...
    def progress(self, step: str, current: int, total: int, description: str = ""):
        ...
        if description:
            ...
            message = (
                f"📊 {step} | [{progress_bar}] {current}/{total} ({percentage:.1f}%)"
            )
    ...
    def api_call_success(self, api_name: str, duration: float = 0, details: str = ""):
        ...
        if duration > 0:
            if details:
                self.info(
                    f"✅ {api_name} API 調用成功 | 耗時: {duration:.2f}s | {details}"
                )
            else:
    ...
    def api_call_error(self, api_name: str, error: str, retry_count: int = 0):
        ...
        if retry_count > 0:
            self.error(
                f"❌ {api_name} API 調用失敗 | 重試次數: {retry_count} | 錯誤: {error}"
            )
        else:
    ...
    def concurrent_operation(
        self, operation: str, current: int, total: int, worker_id: Optional[str] = None
    ):
        """記錄並發操作進度"""
        if worker_id:
            self.debug(
                f"⚡ 併發 {operation} | 工作者: {worker_id} | 進度: {current}/{total}"
            )
        else:
...
def log_api_start(
    api_name: str, endpoint: str = "", model: str = "", details: str = ""
):
    """記錄API調用開始"""
...
def log_concurrent(
    operation: str, current: int, total: int, worker_id: Optional[str] = None
):
    """記錄並發操作"""
